...
document.addEventListener('DOMContentLoaded', () => {
  console.log("dokterrehabilitasi.js loaded!");

  const LARAVEL_URL = "http://localhost:8000"; 
  const searchInput = document.getElementById('searchInput');
  const container = document.getElementById('dokterrehabilitasi-dokterrehabilitasi_m');

  let dokterrehabilitasiData = [];

  function initSection() {
    fetch(`${LARAVEL_URL}/api/dokterrehabilitasi`)
      .then(res => res.json())
      .then(data => {
        dokterrehabilitasiData = data;
        renderCards();
      })
      .catch(err => console.error(`Error fetching dokterrehabilitasi:`, err));
  }

  function renderCards() {
    container.innerHTML = '';

    const itemsToRender = [...dokterrehabilitasiData].sort(
    ...
    container.querySelectorAll('.delete-card').forEach(btn => {
      ...
      btn.addEventListener('click', (e) => {
        ...
        const item = dokterrehabilitasiData.find(b => b.id == id);
        if (!item) return;

        if (confirm('Apakah yakin ingin menghapus dokterrehabilitasi ini?')) {
          fetch(`${LARAVEL_URL}/api/dokterrehabilitasi/${item.id}`, { method: 'DELETE' })
            .then(res => {
              if (res.ok) {
                dokterrehabilitasiData = dokterrehabilitasiData.filter(b => b.id !== item.id);
                renderCards();
                alert('dokterrehabilitasi berhasil dihapus!');
              } else {
                alert('Gagal menghapus dokterrehabilitasi.');
              }
            })
    ...
    container.querySelectorAll('.edit-card').forEach(btn => {
      ...
      btn.addEventListener('click', (e) => {
        ...
        document.getElementById('editdokterrehabilitasiModal').style.display = 'block';
        document.getElementById('editdokterrehabilitasiForm').setAttribute('data-id', item.id);
      });
    });
  }
...
// === Add/Edit Modal ===
const addBtn = document.getElementById('adddokterrehabilitasiBtn');
const modal = document.getElementById('dokterrehabilitasiModal');
const closeBtn = document.getElementById('closedokterrehabilitasiModal');
const form = document.getElementById('dokterrehabilitasiForm');
...
form?.addEventListener('submit', e => {
  e.preventDefault();
  const img = document.getElementById('dokterrehabilitasiImg').files[0];
  const img2 = document.getElementById('dokterrehabilitasiImg2').files[0]; // Gambar tambahan
  ...
  .then(res => res.json())
  .then(newCard => {
    dokterrehabilitasiData.push(newCard);
    renderCards();
    modal.style.display = 'none';
    form.reset();
  })
  .catch(err => {
    console.error(`Error adding dokterrehabilitasi:`, err);
    alert(`Gagal menambahkan data, periksa koneksi backend dokterrehabilitasi.`);
  });
});
...
editModal.id = "editdokterrehabilitasiModal";
...
editModal.innerHTML = `
  ...
  ">
    ...
    <form id="editdokterrehabilitasiForm" style="display: flex; flex-direction: column; gap: 15px;">
      ...
      <textarea id="editTitle" name="title" placeholder="Judul dokterrehabilitasi" required style="
        padding: 10px 12px;
        border: 1px solid #ccc;
        border-radius: 8px;
        font-size: 15px;
...
document.getElementById('editdokterrehabilitasiForm').addEventListener('submit', e => {
  e.preventDefault();
  ...
  fetch(`${LARAVEL_URL}/api/dokterrehabilitasi/${id}`, {
    method: 'POST',
  ...
  .then(res => res.json())
  .then(updated => {
    const idx = dokterrehabilitasiData.findIndex(p => p.id == id);
    dokterrehabilitasiData[idx] = updated;
    renderCards();
    editModal.style.display = "none";
  })
