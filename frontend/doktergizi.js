...
document.addEventListener('DOMContentLoaded', () => {
  console.log("doktergizi.js loaded!");

  const LARAVEL_URL = "http://localhost:8000"; 
  const searchInput = document.getElementById('searchInput');
  const container = document.getElementById('doktergizi-doktergizi_m');

  let doktergiziData = [];

  function initSection() {
    fetch(`${LARAVEL_URL}/api/doktergizi`)
      .then(res => res.json())
      .then(data => {
        doktergiziData = data;
        renderCards();
      })
      .catch(err => console.error(`Error fetching doktergizi:`, err));
  }

  function renderCards() {
    container.innerHTML = '';

    const itemsToRender = [...doktergiziData].sort(
    ...
    container.querySelectorAll('.delete-card').forEach(btn => {
      ...
      btn.addEventListener('click', (e) => {
        ...
        const item = doktergiziData.find(b => b.id == id);
        if (!item) return;

        if (confirm('Apakah yakin ingin menghapus doktergizi ini?')) {
          fetch(`${LARAVEL_URL}/api/doktergizi/${item.id}`, { method: 'DELETE' })
            .then(res => {
              if (res.ok) {
                doktergiziData = doktergiziData.filter(b => b.id !== item.id);
                renderCards();
                alert('doktergizi berhasil dihapus!');
              } else {
                alert('Gagal menghapus doktergizi.');
              }
            })
    ...
    container.querySelectorAll('.edit-card').forEach(btn => {
      ...
      btn.addEventListener('click', (e) => {
        ...
        document.getElementById('editdoktergiziModal').style.display = 'block';
        document.getElementById('editdoktergiziForm').setAttribute('data-id', item.id);
      });
    });
  }
...
// === Add/Edit Modal ===
const addBtn = document.getElementById('adddoktergiziBtn');
const modal = document.getElementById('doktergiziModal');
const closeBtn = document.getElementById('closedoktergiziModal');
const form = document.getElementById('doktergiziForm');
...
form?.addEventListener('submit', e => {
  e.preventDefault();
  const img = document.getElementById('doktergiziImg').files[0];
  const img2 = document.getElementById('doktergiziImg2').files[0]; // Gambar tambahan
  ...
  .then(res => res.json())
  .then(newCard => {
    doktergiziData.push(newCard);
    renderCards();
    modal.style.display = 'none';
    form.reset();
  })
  .catch(err => {
    console.error(`Error adding doktergizi:`, err);
    alert(`Gagal menambahkan data, periksa koneksi backend doktergizi.`);
  });
});
...
editModal.id = "editdoktergiziModal";
...
editModal.innerHTML = `
  ...
  ">
    ...
    <form id="editdoktergiziForm" style="display: flex; flex-direction: column; gap: 15px;">
      ...
      <textarea id="editTitle" name="title" placeholder="Judul doktergizi" required style="
        padding: 10px 12px;
        border: 1px solid #ccc;
        border-radius: 8px;
        font-size: 15px;
...
document.getElementById('editdoktergiziForm').addEventListener('submit', e => {
  e.preventDefault();
  ...
  fetch(`${LARAVEL_URL}/api/doktergizi/${id}`, {
    method: 'POST',
  ...
  .then(res => res.json())
  .then(updated => {
    const idx = doktergiziData.findIndex(p => p.id == id);
    doktergiziData[idx] = updated;
    renderCards();
    editModal.style.display = "none";
  })
