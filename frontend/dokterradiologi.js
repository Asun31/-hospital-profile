...
document.addEventListener('DOMContentLoaded', () => {
  console.log("dokterradiologi.js loaded!");

  const LARAVEL_URL = "http://localhost:8000"; 
  const searchInput = document.getElementById('searchInput');
  const container = document.getElementById('dokterradiologi-dokterradiologi_m');

  let dokterradiologiData = [];

  function initSection() {
    fetch(`${LARAVEL_URL}/api/dokterradiologi`)
      .then(res => res.json())
      .then(data => {
        dokterradiologiData = data;
        renderCards();
      })
      .catch(err => console.error(`Error fetching dokterradiologi:`, err));
  }

  function renderCards() {
    container.innerHTML = '';

    const itemsToRender = [...dokterradiologiData].sort(
    ...
    container.querySelectorAll('.delete-card').forEach(btn => {
      ...
      btn.addEventListener('click', (e) => {
        ...
        const item = dokterradiologiData.find(b => b.id == id);
        if (!item) return;

        if (confirm('Apakah yakin ingin menghapus dokterradiologi ini?')) {
          fetch(`${LARAVEL_URL}/api/dokterradiologi/${item.id}`, { method: 'DELETE' })
            .then(res => {
              if (res.ok) {
                dokterradiologiData = dokterradiologiData.filter(b => b.id !== item.id);
                renderCards();
                alert('dokterradiologi berhasil dihapus!');
              } else {
                alert('Gagal menghapus dokterradiologi.');
              }
            })
    ...
    container.querySelectorAll('.edit-card').forEach(btn => {
      ...
      btn.addEventListener('click', (e) => {
        ...
        document.getElementById('editdokterradiologiModal').style.display = 'block';
        document.getElementById('editdokterradiologiForm').setAttribute('data-id', item.id);
      });
    });
  }
...
// === Add/Edit Modal ===
const addBtn = document.getElementById('adddokterradiologiBtn');
const modal = document.getElementById('dokterradiologiModal');
const closeBtn = document.getElementById('closedokterradiologiModal');
const form = document.getElementById('dokterradiologiForm');
...
form?.addEventListener('submit', e => {
  e.preventDefault();
  const img = document.getElementById('dokterradiologiImg').files[0];
  const img2 = document.getElementById('dokterradiologiImg2').files[0]; // Gambar tambahan
  ...
  .then(res => res.json())
  .then(newCard => {
    dokterradiologiData.push(newCard);
    renderCards();
    modal.style.display = 'none';
    form.reset();
  })
  .catch(err => {
    console.error(`Error adding dokterradiologi:`, err);
    alert(`Gagal menambahkan data, periksa koneksi backend dokterradiologi.`);
  });
});
...
editModal.id = "editdokterradiologiModal";
...
editModal.innerHTML = `
  ...
  ">
    ...
    <form id="editdokterradiologiForm" style="display: flex; flex-direction: column; gap: 15px;">
      ...
      <textarea id="editTitle" name="title" placeholder="Judul dokterradiologi" required style="
        padding: 10px 12px;
        border: 1px solid #ccc;
        border-radius: 8px;
        font-size: 15px;
...
document.getElementById('editdokterradiologiForm').addEventListener('submit', e => {
  e.preventDefault();
  ...
  fetch(`${LARAVEL_URL}/api/dokterradiologi/${id}`, {
    method: 'POST',
  ...
  .then(res => res.json())
  .then(updated => {
    const idx = dokterradiologiData.findIndex(p => p.id == id);
    dokterradiologiData[idx] = updated;
    renderCards();
    editModal.style.display = "none";
  })
