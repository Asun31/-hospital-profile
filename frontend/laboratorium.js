...
document.addEventListener('DOMContentLoaded', () => {
  console.log("laboratorium.js loaded!");

  const LARAVEL_URL = "http://localhost:8000";
  const container = document.getElementById('laboratorium-laboratorium_m');
  const searchInput = document.getElementById('searchInput');
  const addBtn = document.getElementById('addlaboratoriumBtn');
  const addModal = document.getElementById('laboratoriumModal');
  const closeAddBtn = document.getElementById('closelaboratoriumModal');
  const addForm = document.getElementById('laboratoriumForm');

  let laboratoriumData = [];
  ...
  const fetchlaboratorium = async () => {
    try {
      const res = await fetch(`${LARAVEL_URL}/api/laboratorium`);
      laboratoriumData = await res.json();
      renderCards();
    } catch (err) {
      console.error("Error fetching laboratorium:", err);
    }
  };

  const addlaboratorium = async form => {
    try {
      const formData = new FormData(form);
      const res = await fetch(`${LARAVEL_URL}/api/laboratorium`, { method: 'POST', body: formData });
      const newCard = await res.json();
      laboratoriumData.push(newCard);
      renderCards();
      closeModal(addModal);
      form.reset();
    } catch (err) {
      console.error("Error adding laboratorium:", err);
      alert("Gagal menambahkan data, periksa koneksi backend laboratorium.");
    }
  };

  const updatelaboratorium = async (id, form) => {
    try {
      const formData = new FormData(form);
      const imgFile = document.getElementById('editImg').files[0];
      if (imgFile) formData.append('img', imgFile);

      const res = await fetch(`${LARAVEL_URL}/api/laboratorium/${id}`, {
        method: 'POST',
        headers: { 'X-HTTP-Method-Override': 'PUT' },
        body: formData
      });
      const updated = await res.json();
      const idx = laboratoriumData.findIndex(p => p.id == id);
      laboratoriumData[idx] = updated;
      renderCards();
  ...
  const deletelaboratorium = async id => {
    try {
      const res = await fetch(`${LARAVEL_URL}/api/laboratorium/${id}`, { method: 'DELETE' });
      if (res.ok) {
        laboratoriumData = laboratoriumData.filter(b => b.id != id);
        renderCards();
        alert('laboratorium berhasil dihapus!');
      } else alert('Gagal menghapus laboratorium.');
    } catch (err) {
      console.error(err);
    }
  };
  ...
  const renderCards = () => {
    container.innerHTML = laboratoriumData
  ...
  editModal.id = "editlaboratoriumModal";
  ...
  editModal.innerHTML = `
    <div style="background:white; max-width:520px; margin:60px auto; padding:25px 30px; border-radius:12px; position:relative; box-shadow:0 8px 30px rgba(0,0,0,0.2); font-family:'Segoe UI', Tahoma, sans-serif;">
      ...
      <h3 style="text-align:center;font-size:20px;font-weight:bold;color:#2c3e50;margin-bottom:20px;">✏️ Edit laboratorium</h3>
      <form id="editlaboratoriumForm" style="display:flex;flex-direction:column;gap:15px;">
        <textarea id="editTitle" name="title" placeholder="Judul laboratorium" required style="padding:10px 12px;border:1px solid #ccc;border-radius:8px;font-size:15px;font-weight:600;resize:none;min-height:60px;line-height:1.4;"></textarea>
  ...
  const handleEdit = id => {
    const item = laboratoriumData.find(b => b.id == id);
    if (!item) return;
    document.getElementById('editTitle').value = item.title;
    document.getElementById('editContent').value = item.content;
    document.getElementById('editlaboratoriumForm').setAttribute('data-id', item.id);
    openModal(editModal);
  };

  const handleDelete = id => {
    if (confirm('Apakah yakin ingin menghapus laboratorium ini?')) {
      deletelaboratorium(id);
    }
  };

  document.getElementById('editlaboratoriumForm').addEventListener('submit', e => {
    e.preventDefault();
    if (!confirm("Yakin mau menyimpan perubahan?")) return;
    const form = e.target;
    const id = form.getAttribute('data-id');
    updatelaboratorium(id, form);
  });
