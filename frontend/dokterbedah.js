...
document.addEventListener('DOMContentLoaded', () => {
  console.log("dokterbedah.js loaded!");

  const LARAVEL_URL = "http://localhost:8000"; 
  const searchInput = document.getElementById('searchInput');
  const container = document.getElementById('dokterbedah-dokterbedah_m');

  let dokterbedahData = [];

  function initSection() {
    fetch(`${LARAVEL_URL}/api/dokterbedah`)
      .then(res => res.json())
      .then(data => {
        dokterbedahData = data;
        renderCards();
      })
      .catch(err => console.error(`Error fetching dokterbedah:`, err));
  }

  function renderCards() {
    container.innerHTML = '';

    const itemsToRender = [...dokterbedahData].sort(
    ...
    container.querySelectorAll('.delete-card').forEach(btn => {
      ...
      btn.addEventListener('click', (e) => {
        ...
        const item = dokterbedahData.find(b => b.id == id);
        if (!item) return;

        if (confirm('Apakah yakin ingin menghapus dokterbedah ini?')) {
          fetch(`${LARAVEL_URL}/api/dokterbedah/${item.id}`, { method: 'DELETE' })
            .then(res => {
              if (res.ok) {
                dokterbedahData = dokterbedahData.filter(b => b.id !== item.id);
                renderCards();
                alert('dokterbedah berhasil dihapus!');
              } else {
                alert('Gagal menghapus dokterbedah.');
              }
            })
    ...
    container.querySelectorAll('.edit-card').forEach(btn => {
      ...
      btn.addEventListener('click', (e) => {
        ...
        document.getElementById('editdokterbedahModal').style.display = 'block';
        document.getElementById('editdokterbedahForm').setAttribute('data-id', item.id);
      });
    });
  }
...
// === Add/Edit Modal ===
const addBtn = document.getElementById('adddokterbedahBtn');
const modal = document.getElementById('dokterbedahModal');
const closeBtn = document.getElementById('closedokterbedahModal');
const form = document.getElementById('dokterbedahForm');
...
form?.addEventListener('submit', e => {
  e.preventDefault();
  const img = document.getElementById('dokterbedahImg').files[0];
  const img2 = document.getElementById('dokterbedahImg2').files[0]; // Gambar tambahan
  ...
  .then(res => res.json())
  .then(newCard => {
    dokterbedahData.push(newCard);
    renderCards();
    modal.style.display = 'none';
    form.reset();
  })
  .catch(err => {
    console.error(`Error adding dokterbedah:`, err);
    alert(`Gagal menambahkan data, periksa koneksi backend dokterbedah.`);
  });
});
...
editModal.id = "editdokterbedahModal";
...
editModal.innerHTML = `
  ...
  ">
    ...
    <form id="editdokterbedahForm" style="display: flex; flex-direction: column; gap: 15px;">
      ...
      <textarea id="editTitle" name="title" placeholder="Judul dokterbedah" required style="
        padding: 10px 12px;
        border: 1px solid #ccc;
        border-radius: 8px;
        font-size: 15px;
...
document.getElementById('editdokterbedahForm').addEventListener('submit', e => {
  e.preventDefault();
  ...
  fetch(`${LARAVEL_URL}/api/dokterbedah/${id}`, {
    method: 'POST',
  ...
  .then(res => res.json())
  .then(updated => {
    const idx = dokterbedahData.findIndex(p => p.id == id);
    dokterbedahData[idx] = updated;
    renderCards();
    editModal.style.display = "none";
  })
