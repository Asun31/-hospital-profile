...
document.addEventListener('DOMContentLoaded', () => {
  console.log("doktertht.js loaded!");

  const LARAVEL_URL = "http://localhost:8000"; 
  const searchInput = document.getElementById('searchInput');
  const container = document.getElementById('doktertht-doktertht_m');

  let dokterthtData = [];

  function initSection() {
    fetch(`${LARAVEL_URL}/api/doktertht`)
      .then(res => res.json())
      .then(data => {
        dokterthtData = data;
        renderCards();
      })
      .catch(err => console.error(`Error fetching doktertht:`, err));
  }

  function renderCards() {
    container.innerHTML = '';

    const itemsToRender = [...dokterthtData].sort(
    ...
    container.querySelectorAll('.delete-card').forEach(btn => {
      ...
      btn.addEventListener('click', (e) => {
        ...
        const item = dokterthtData.find(b => b.id == id);
        if (!item) return;

        if (confirm('Apakah yakin ingin menghapus doktertht ini?')) {
          fetch(`${LARAVEL_URL}/api/doktertht/${item.id}`, { method: 'DELETE' })
            .then(res => {
              if (res.ok) {
                dokterthtData = dokterthtData.filter(b => b.id !== item.id);
                renderCards();
                alert('doktertht berhasil dihapus!');
              } else {
                alert('Gagal menghapus doktertht.');
              }
            })
    ...
    container.querySelectorAll('.edit-card').forEach(btn => {
      ...
      btn.addEventListener('click', (e) => {
        ...
        document.getElementById('editdokterthtModal').style.display = 'block';
        document.getElementById('editdokterthtForm').setAttribute('data-id', item.id);
      });
    });
  }
...
// === Add/Edit Modal ===
const addBtn = document.getElementById('adddokterthtBtn');
const modal = document.getElementById('dokterthtModal');
const closeBtn = document.getElementById('closedokterthtModal');
const form = document.getElementById('dokterthtForm');
...
form?.addEventListener('submit', e => {
  e.preventDefault();
  const img = document.getElementById('dokterthtImg').files[0];
  const img2 = document.getElementById('dokterthtImg2').files[0]; // Gambar tambahan
  ...
  .then(res => res.json())
  .then(newCard => {
    dokterthtData.push(newCard);
    renderCards();
    modal.style.display = 'none';
    form.reset();
  })
  .catch(err => {
    console.error(`Error adding doktertht:`, err);
    alert(`Gagal menambahkan data, periksa koneksi backend doktertht.`);
  });
});
...
editModal.id = "editdokterthtModal";
...
editModal.innerHTML = `
  ...
  ">
    ...
    <form id="editdokterthtForm" style="display: flex; flex-direction: column; gap: 15px;">
      ...
      <textarea id="editTitle" name="title" placeholder="Judul doktertht" required style="
        padding: 10px 12px;
        border: 1px solid #ccc;
        border-radius: 8px;
        font-size: 15px;
...
document.getElementById('editdokterthtForm').addEventListener('submit', e => {
  e.preventDefault();
  ...
  fetch(`${LARAVEL_URL}/api/doktertht/${id}`, {
    method: 'POST',
  ...
  .then(res => res.json())
  .then(updated => {
    const idx = dokterthtData.findIndex(p => p.id == id);
    dokterthtData[idx] = updated;
    renderCards();
    editModal.style.display = "none";
  })
