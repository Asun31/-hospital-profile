...
document.addEventListener('DOMContentLoaded', () => {
  console.log("rawatinap.js loaded!");

  const LARAVEL_URL = "http://localhost:8000";
  const container = document.getElementById('rawatinap-rawatinap_m');
  const searchInput = document.getElementById('searchInput');
  const addBtn = document.getElementById('addrawatinapBtn');
  const addModal = document.getElementById('rawatinapModal');
  const closeAddBtn = document.getElementById('closerawatinapModal');
  const addForm = document.getElementById('rawatinapForm');
  ...
  const fetchrawatinap = async () => {
    try {
      const res = await fetch(`${LARAVEL_URL}/api/rawatinap`);
      rawatinapData = await res.json();
      renderCards();
    } catch (err) {
      console.error("Error fetching rawatinap:", err);
    }
  };

  const addrawatinap = async form => {
    try {
      const formData = new FormData(form);
      const res = await fetch(`${LARAVEL_URL}/api/rawatinap`, { method: 'POST', body: formData });
      const newCard = await res.json();
      rawatinapData.push(newCard);
      renderCards();
      closeModal(addModal);
      form.reset();
    } catch (err) {
      console.error("Error adding rawatinap:", err);
      alert("Gagal menambahkan data, periksa koneksi backend rawatinap.");
    }
  };

  const updaterawatinap = async (id, form) => {
    try {
      const formData = new FormData(form);
      const imgFile = document.getElementById('editImg').files[0];
      if (imgFile) formData.append('img', imgFile);

      const res = await fetch(`${LARAVEL_URL}/api/rawatinap/${id}`, {
        method: 'POST',
        headers: { 'X-HTTP-Method-Override': 'PUT' },
        body: formData
      });
      const updated = await res.json();
      const idx = rawatinapData.findIndex(p => p.id == id);
      rawatinapData[idx] = updated;
      renderCards();
  ...
  const deleterawatinap = async id => {
    try {
      const res = await fetch(`${LARAVEL_URL}/api/rawatinap/${id}`, { method: 'DELETE' });
      if (res.ok) {
        rawatinapData = rawatinapData.filter(b => b.id != id);
        renderCards();
        alert('rawatinap berhasil dihapus!');
      } else alert('Gagal menghapus rawatinap.');
    } catch (err) {
      console.error(err);
    }
  };
  ...
  const renderCards = () => {
    container.innerHTML = rawatinapData
  ...
  editModal.id = "editrawatinapModal";
  ...
  editModal.innerHTML = `
    <div style="background:white; max-width:520px; margin:60px auto; padding:25px 30px; border-radius:12px; position:relative; box-shadow:0 8px 30px rgba(0,0,0,0.2); font-family:'Segoe UI', Tahoma, sans-serif;">
      ...
      <h3 style="text-align:center;font-size:20px;font-weight:bold;color:#2c3e50;margin-bottom:20px;">✏️ Edit rawatinap</h3>
      <form id="editrawatinapForm" style="display:flex;flex-direction:column;gap:15px;">
        <textarea id="editTitle" name="title" placeholder="Judul rawatinap" required style="padding:10px 12px;border:1px solid #ccc;border-radius:8px;font-size:15px;font-weight:600;resize:none;min-height:60px;line-height:1.4;"></textarea>
  ...
  const handleEdit = id => {
    const item = rawatinapData.find(b => b.id == id);
    if (!item) return;
    document.getElementById('editTitle').value = item.title;
    document.getElementById('editContent').value = item.content;
    document.getElementById('editrawatinapForm').setAttribute('data-id', item.id);
    openModal(editModal);
  };

  const handleDelete = id => {
    if (confirm('Apakah yakin ingin menghapus rawatinap ini?')) {
      deleterawatinap(id);
    }
  };

  document.getElementById('editrawatinapForm').addEventListener('submit', e => {
    e.preventDefault();
    if (!confirm("Yakin mau menyimpan perubahan?")) return;
    const form = e.target;
    const id = form.getAttribute('data-id');
    updaterawatinap(id, form);
  });
