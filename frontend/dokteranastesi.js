...
document.addEventListener('DOMContentLoaded', () => {
  console.log("dokteranastesi.js loaded!");

  const LARAVEL_URL = "http://localhost:8000"; 
  const searchInput = document.getElementById('searchInput');
  const container = document.getElementById('dokteranastesi-dokteranastesi_m');

  let dokteranastesiData = [];

  function initSection() {
    fetch(`${LARAVEL_URL}/api/dokteranastesi`)
      .then(res => res.json())
      .then(data => {
        dokteranastesiData = data;
        renderCards();
      })
      .catch(err => console.error(`Error fetching dokteranastesi:`, err));
  }

  function renderCards() {
    container.innerHTML = '';

    const itemsToRender = [...dokteranastesiData].sort(
    ...
    container.querySelectorAll('.delete-card').forEach(btn => {
      ...
      btn.addEventListener('click', (e) => {
        ...
        const item = dokteranastesiData.find(b => b.id == id);
        if (!item) return;

        if (confirm('Apakah yakin ingin menghapus dokteranastesi ini?')) {
          fetch(`${LARAVEL_URL}/api/dokteranastesi/${item.id}`, { method: 'DELETE' })
            .then(res => {
              if (res.ok) {
                dokteranastesiData = dokteranastesiData.filter(b => b.id !== item.id);
                renderCards();
                alert('dokteranastesi berhasil dihapus!');
              } else {
                alert('Gagal menghapus dokteranastesi.');
              }
            })
    ...
    container.querySelectorAll('.edit-card').forEach(btn => {
      ...
      btn.addEventListener('click', (e) => {
        ...
        document.getElementById('editdokteranastesiModal').style.display = 'block';
        document.getElementById('editdokteranastesiForm').setAttribute('data-id', item.id);
      });
    });
  }
...
// === Add/Edit Modal ===
const addBtn = document.getElementById('adddokteranastesiBtn');
const modal = document.getElementById('dokteranastesiModal');
const closeBtn = document.getElementById('closedokteranastesiModal');
const form = document.getElementById('dokteranastesiForm');
...
form?.addEventListener('submit', e => {
  e.preventDefault();
  const img = document.getElementById('dokteranastesiImg').files[0];
  const img2 = document.getElementById('dokteranastesiImg2').files[0]; // Gambar tambahan
  ...
  .then(res => res.json())
  .then(newCard => {
    dokteranastesiData.push(newCard);
    renderCards();
    modal.style.display = 'none';
    form.reset();
  })
  .catch(err => {
    console.error(`Error adding dokteranastesi:`, err);
    alert(`Gagal menambahkan data, periksa koneksi backend dokteranastesi.`);
  });
});
...
editModal.id = "editdokteranastesiModal";
...
editModal.innerHTML = `
  ...
  ">
    ...
    <form id="editdokteranastesiForm" style="display: flex; flex-direction: column; gap: 15px;">
      ...
      <textarea id="editTitle" name="title" placeholder="Judul dokteranastesi" required style="
        padding: 10px 12px;
        border: 1px solid #ccc;
        border-radius: 8px;
        font-size: 15px;
...
document.getElementById('editdokteranastesiForm').addEventListener('submit', e => {
  e.preventDefault();
  ...
  fetch(`${LARAVEL_URL}/api/dokteranastesi/${id}`, {
    method: 'POST',
  ...
  .then(res => res.json())
  .then(updated => {
    const idx = dokteranastesiData.findIndex(p => p.id == id);
    dokteranastesiData[idx] = updated;
    renderCards();
    editModal.style.display = "none";
  })
