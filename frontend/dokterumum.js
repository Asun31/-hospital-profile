...
document.addEventListener('DOMContentLoaded', () => {
  console.log("dokterumum.js loaded!");

  const LARAVEL_URL = "http://localhost:8000"; 
  const searchInput = document.getElementById('searchInput');
  const container = document.getElementById('dokterumum-dokterumum_m');

  let dokterumumData = [];

  function initSection() {
    fetch(`${LARAVEL_URL}/api/dokterumum`)
      .then(res => res.json())
      .then(data => {
        dokterumumData = data;
        renderCards();
      })
      .catch(err => console.error(`Error fetching dokterumum:`, err));
  }

  function renderCards() {
    container.innerHTML = '';

    const itemsToRender = [...dokterumumData].sort(
    ...
    container.querySelectorAll('.delete-card').forEach(btn => {
      ...
      btn.addEventListener('click', (e) => {
        ...
        const item = dokterumumData.find(b => b.id == id);
        if (!item) return;

        if (confirm('Apakah yakin ingin menghapus dokterumum ini?')) {
          fetch(`${LARAVEL_URL}/api/dokterumum/${item.id}`, { method: 'DELETE' })
            .then(res => {
              if (res.ok) {
                dokterumumData = dokterumumData.filter(b => b.id !== item.id);
                renderCards();
                alert('dokterumum berhasil dihapus!');
              } else {
                alert('Gagal menghapus dokterumum.');
              }
            })
    ...
    container.querySelectorAll('.edit-card').forEach(btn => {
      ...
      btn.addEventListener('click', (e) => {
        ...
        document.getElementById('editdokterumumModal').style.display = 'block';
        document.getElementById('editdokterumumForm').setAttribute('data-id', item.id);
      });
    });
  }
...
// === Add/Edit Modal ===
const addBtn = document.getElementById('adddokterumumBtn');
const modal = document.getElementById('dokterumumModal');
const closeBtn = document.getElementById('closedokterumumModal');
const form = document.getElementById('dokterumumForm');
...
form?.addEventListener('submit', e => {
  e.preventDefault();
  const img = document.getElementById('dokterumumImg').files[0];
  const img2 = document.getElementById('dokterumumImg2').files[0]; // Gambar tambahan
  ...
  .then(res => res.json())
  .then(newCard => {
    dokterumumData.push(newCard);
    renderCards();
    modal.style.display = 'none';
    form.reset();
  })
  .catch(err => {
    console.error(`Error adding dokterumum:`, err);
    alert(`Gagal menambahkan data, periksa koneksi backend dokterumum.`);
  });
});
...
editModal.id = "editdokterumumModal";
...
editModal.innerHTML = `
  ...
  ">
    ...
    <form id="editdokterumumForm" style="display: flex; flex-direction: column; gap: 15px;">
      ...
      <textarea id="editTitle" name="title" placeholder="Judul dokterumum" required style="
        padding: 10px 12px;
        border: 1px solid #ccc;
        border-radius: 8px;
        font-size: 15px;
...
document.getElementById('editdokterumumForm').addEventListener('submit', e => {
  e.preventDefault();
  ...
  fetch(`${LARAVEL_URL}/api/dokterumum/${id}`, {
    method: 'POST',
  ...
  .then(res => res.json())
  .then(updated => {
    const idx = dokterumumData.findIndex(p => p.id == id);
    dokterumumData[idx] = updated;
    renderCards();
    editModal.style.display = "none";
  })
