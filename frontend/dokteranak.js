...
document.addEventListener('DOMContentLoaded', () => {
  console.log("dokteranak.js loaded!");

  const LARAVEL_URL = "http://localhost:8000"; 
  const searchInput = document.getElementById('searchInput');
  const container = document.getElementById('dokteranak-dokteranak_m');
  ...
  function initSection() {
    fetch(`${LARAVEL_URL}/api/dokteranak`)
      ...
      .catch(err => console.error(`Error fetching dokteranak:`, err));
  }

  function renderCards() {
    container.innerHTML = '';

    const itemsToRender = [...dokteranakData].sort(
    ...
    container.querySelectorAll('.delete-card').forEach(btn => {
      ...
      btn.addEventListener('click', (e) => {
        ...
        if (confirm('Apakah yakin ingin menghapus dokteranak ini?')) {
          fetch(`${LARAVEL_URL}/api/dokteranak/${item.id}`, { method: 'DELETE' })
            .then(res => {
              if (res.ok) {
                dokteranakData = dokteranakData.filter(b => b.id !== item.id);
                renderCards();
                alert('dokteranak berhasil dihapus!');
              } else {
                alert('Gagal menghapus dokteranak.');
              }
            })
    ...
    container.querySelectorAll('.edit-card').forEach(btn => {
      ...
      btn.addEventListener('click', (e) => {
        ...
        document.getElementById('editdokteranakModal').style.display = 'block';
        document.getElementById('editdokteranakForm').setAttribute('data-id', item.id);
      });
    });
  }
