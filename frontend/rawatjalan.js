...
document.addEventListener('DOMContentLoaded', () => {
  console.log("rawatjalan.js loaded!");

  const LARAVEL_URL = "http://localhost:8000";
  const container = document.getElementById('rawatjalan-rawatjalan_m');
  const searchInput = document.getElementById('searchInput');
  const addBtn = document.getElementById('addrawatjalanBtn');
  const addModal = document.getElementById('rawatjalanModal');
  const closeAddBtn = document.getElementById('closerawatjalanModal');
  const addForm = document.getElementById('rawatjalanForm');
  ...
  const fetchRawatjalan = async () => {
    try {
      const res = await fetch(`${LARAVEL_URL}/api/rawatjalan`);
      rawatjalanData = await res.json();
      renderCards();
    } catch (err) {
      console.error("Error fetching rawatjalan:", err);
    }
  };

  const addRawatjalan = async form => {
    try {
      const formData = new FormData(form);
      const res = await fetch(`${LARAVEL_URL}/api/rawatjalan`, { method: 'POST', body: formData });
      const newCard = await res.json();
      rawatjalanData.push(newCard);
      renderCards();
      closeModal(addModal);
      form.reset();
    } catch (err) {
      console.error("Error adding rawatjalan:", err);
      alert("Gagal menambahkan data, periksa koneksi backend rawatjalan.");
    }
  };

  const updateRawatjalan = async (id, form) => {
    try {
      const formData = new FormData(form);
      const imgFile = document.getElementById('editImg').files[0];
      if (imgFile) formData.append('img', imgFile);

      const res = await fetch(`${LARAVEL_URL}/api/rawatjalan/${id}`, {
        method: 'POST',
        headers: { 'X-HTTP-Method-Override': 'PUT' },
        body: formData
      });
      const updated = await res.json();
      const idx = rawatjalanData.findIndex(p => p.id == id);
      rawatjalanData[idx] = updated;
      renderCards();
  ...
  const deleteRawatjalan = async id => {
    try {
      const res = await fetch(`${LARAVEL_URL}/api/rawatjalan/${id}`, { method: 'DELETE' });
      if (res.ok) {
        rawatjalanData = rawatjalanData.filter(b => b.id != id);
        renderCards();
        alert('rawatjalan berhasil dihapus!');
      } else alert('Gagal menghapus rawatjalan.');
    } catch (err) {
      console.error(err);
    }
  };
  ...
  const renderCards = () => {
    container.innerHTML = rawatjalanData
  ...
  editModal.id = "editrawatjalanModal";
  ...
  editModal.innerHTML = `
    <div style="background:white; max-width:520px; margin:60px auto; padding:25px 30px; border-radius:12px; position:relative; box-shadow:0 8px 30px rgba(0,0,0,0.2); font-family:'Segoe UI', Tahoma, sans-serif;">
      ...
      <h3 style="text-align:center;font-size:20px;font-weight:bold;color:#2c3e50;margin-bottom:20px;">✏️ Edit rawatjalan</h3>
      <form id="editrawatjalanForm" style="display:flex;flex-direction:column;gap:15px;">
        <textarea id="editTitle" name="title" placeholder="Judul rawatjalan" required style="padding:10px 12px;border:1px solid #ccc;border-radius:8px;font-size:15px;font-weight:600;resize:none;min-height:60px;line-height:1.4;"></textarea>
  ...
  const handleEdit = id => {
    const item = rawatjalanData.find(b => b.id == id);
    if (!item) return;
    document.getElementById('editTitle').value = item.title;
    document.getElementById('editContent').value = item.content;
    document.getElementById('editrawatjalanForm').setAttribute('data-id', item.id);
    openModal(editModal);
  };

  const handleDelete = id => {
    if (confirm('Apakah yakin ingin menghapus rawatjalan ini?')) {
      deleteRawatjalan(id);
    }
  };

  document.getElementById('editrawatjalanForm').addEventListener('submit', e => {
    e.preventDefault();
    if (!confirm("Yakin mau menyimpan perubahan?")) return;
    const form = e.target;
    const id = form.getAttribute('data-id');
    updateRawatjalan(id, form);
  });
