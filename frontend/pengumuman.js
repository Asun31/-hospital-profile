...
document.addEventListener('DOMContentLoaded', () => {
  console.log("pengumuman.js loaded!");

  const LARAVEL_URL = "http://localhost:8000"; 
  const searchInput = document.getElementById('searchInput');
  const container = document.getElementById('pengumuman-pengumuman_m');
  ...
  function renderCards() {
    ...
    container.querySelectorAll('.delete-card').forEach(btn => {
      ...
      btn.addEventListener('click', (e) => {
        ...
        const item = pengumumanData.find(b => b.id == id);
        if (!item) return;
    ...
    detailModal.innerHTML = `
      ...
      ">
        ...
        <div style="
          ...
          overflow: hidden;
        ">
          <img id="detailImage" src="" alt="Gambar pengumuman" style="
            max-width: 100%;
            max-height: 100%;
            object-fit: contain;
            display: block;
  ...
  const closeBtn = document.getElementById('closepengumumanModal');
  ...
  form?.addEventListener('submit', e => {
    ...
    .then(res => res.json())
    .then(newCard => {
      pengumumanData.push(newCard);
      renderCards();
      modal.style.display = 'none';
      form.reset();
    })
    .catch(err => {
      console.error(`Error adding pengumuman:`, err);
      alert(`Gagal menambahkan data, periksa koneksi backend pengumuman.`);
    });
  });
  ...
  editModal.id = "editpengumumanModal";
  ...
  editModal.innerHTML = `
    ...
    ">
      ...
      <h3 style="
        ...
        color: #2c3e50;
        margin-bottom: 20px;
      ">✏️ Edit pengumuman</h3>

      <form id="editpengumumanForm" style="display: flex; flex-direction: column; gap: 15px;">
        <textarea id="editTitle" name="title" placeholder="Judul pengumuman" required style="
          padding: 10px 12px;
  ...
  document.getElementById('editpengumumanForm').addEventListener('submit', e => {
    e.preventDefault();
    ...
    fetch(`${LARAVEL_URL}/api/pengumuman/${id}`, {
      method: 'POST',
    ...
    .then(res => res.json())
    .then(updated => {
      const idx = pengumumanData.findIndex(p => p.id == id);
      pengumumanData[idx] = updated;
      renderCards();
      editModal.style.display = "none";
    })
