...
document.addEventListener('DOMContentLoaded', () => {
  console.log("dokterpenyakitdalam.js loaded!");

  const LARAVEL_URL = "http://localhost:8000"; 
  const searchInput = document.getElementById('searchInput');
  const container = document.getElementById('dokterpenyakitdalam-dokterpenyakitdalam_m');

  let dokterpenyakitdalamData = [];

  function initSection() {
    fetch(`${LARAVEL_URL}/api/dokterpenyakitdalam`)
      .then(res => res.json())
      .then(data => {
        dokterpenyakitdalamData = data;
        renderCards();
      })
      .catch(err => console.error(`Error fetching dokterpenyakitdalam:`, err));
  }

  function renderCards() {
    container.innerHTML = '';

    const itemsToRender = [...dokterpenyakitdalamData].sort(
    ...
    container.querySelectorAll('.delete-card').forEach(btn => {
      ...
      btn.addEventListener('click', (e) => {
        ...
        const item = dokterpenyakitdalamData.find(b => b.id == id);
        if (!item) return;

        if (confirm('Apakah yakin ingin menghapus dokterpenyakitdalam ini?')) {
          fetch(`${LARAVEL_URL}/api/dokterpenyakitdalam/${item.id}`, { method: 'DELETE' })
            .then(res => {
              if (res.ok) {
                dokterpenyakitdalamData = dokterpenyakitdalamData.filter(b => b.id !== item.id);
                renderCards();
                alert('dokterpenyakitdalam berhasil dihapus!');
              } else {
                alert('Gagal menghapus dokterpenyakitdalam.');
              }
            })
    ...
    container.querySelectorAll('.edit-card').forEach(btn => {
      ...
      btn.addEventListener('click', (e) => {
        ...
        document.getElementById('editdokterpenyakitdalamModal').style.display = 'block';
        document.getElementById('editdokterpenyakitdalamForm').setAttribute('data-id', item.id);
      });
    });
  }
...
// === Add/Edit Modal ===
const addBtn = document.getElementById('adddokterpenyakitdalamBtn');
const modal = document.getElementById('dokterpenyakitdalamModal');
const closeBtn = document.getElementById('closedokterpenyakitdalamModal');
const form = document.getElementById('dokterpenyakitdalamForm');
...
form?.addEventListener('submit', e => {
  e.preventDefault();
  const img = document.getElementById('dokterpenyakitdalamImg').files[0];
  const img2 = document.getElementById('dokterpenyakitdalamImg2').files[0]; // Gambar tambahan
  ...
  .then(res => res.json())
  .then(newCard => {
    dokterpenyakitdalamData.push(newCard);
    renderCards();
    modal.style.display = 'none';
    form.reset();
  })
  .catch(err => {
    console.error(`Error adding dokterpenyakitdalam:`, err);
    alert(`Gagal menambahkan data, periksa koneksi backend dokterpenyakitdalam.`);
  });
});
...
editModal.id = "editdokterpenyakitdalamModal";
...
editModal.innerHTML = `
  ...
  ">
    ...
    <form id="editdokterpenyakitdalamForm" style="display: flex; flex-direction: column; gap: 15px;">
      ...
      <textarea id="editTitle" name="title" placeholder="Judul dokterpenyakitdalam" required style="
        padding: 10px 12px;
        border: 1px solid #ccc;
        border-radius: 8px;
        font-size: 15px;
...
document.getElementById('editdokterpenyakitdalamForm').addEventListener('submit', e => {
  e.preventDefault();
  ...
  fetch(`${LARAVEL_URL}/api/dokterpenyakitdalam/${id}`, {
    method: 'POST',
  ...
  .then(res => res.json())
  .then(updated => {
    const idx = dokterpenyakitdalamData.findIndex(p => p.id == id);
    dokterpenyakitdalamData[idx] = updated;
    renderCards();
    editModal.style.display = "none";
  })
