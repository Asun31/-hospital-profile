...
document.addEventListener('DOMContentLoaded', () => {
  console.log("doktersaraf.js loaded!");

  const LARAVEL_URL = "http://localhost:8000"; 
  const searchInput = document.getElementById('searchInput');
  const container = document.getElementById('doktersaraf-doktersaraf_m');

  let doktersarafData = [];

  function initSection() {
    fetch(`${LARAVEL_URL}/api/doktersaraf`)
      .then(res => res.json())
      .then(data => {
        doktersarafData = data;
        renderCards();
      })
      .catch(err => console.error(`Error fetching doktersaraf:`, err));
  }

  function renderCards() {
    container.innerHTML = '';

    const itemsToRender = [...doktersarafData].sort(
    ...
    container.querySelectorAll('.delete-card').forEach(btn => {
      ...
      btn.addEventListener('click', (e) => {
        ...
        const item = doktersarafData.find(b => b.id == id);
        if (!item) return;

        if (confirm('Apakah yakin ingin menghapus doktersaraf ini?')) {
          fetch(`${LARAVEL_URL}/api/doktersaraf/${item.id}`, { method: 'DELETE' })
            .then(res => {
              if (res.ok) {
                doktersarafData = doktersarafData.filter(b => b.id !== item.id);
                renderCards();
                alert('doktersaraf berhasil dihapus!');
              } else {
                alert('Gagal menghapus doktersaraf.');
              }
            })
    ...
    container.querySelectorAll('.edit-card').forEach(btn => {
      ...
      btn.addEventListener('click', (e) => {
        ...
        document.getElementById('editdoktersarafModal').style.display = 'block';
        document.getElementById('editdoktersarafForm').setAttribute('data-id', item.id);
      });
    });
  }
...
// === Add/Edit Modal ===
const addBtn = document.getElementById('adddoktersarafBtn');
const modal = document.getElementById('doktersarafModal');
const closeBtn = document.getElementById('closedoktersarafModal');
const form = document.getElementById('doktersarafForm');
...
form?.addEventListener('submit', e => {
  e.preventDefault();
  const img = document.getElementById('doktersarafImg').files[0];
  const img2 = document.getElementById('doktersarafImg2').files[0]; // Gambar tambahan
  ...
  .then(res => res.json())
  .then(newCard => {
    doktersarafData.push(newCard);
    renderCards();
    modal.style.display = 'none';
    form.reset();
  })
  .catch(err => {
    console.error(`Error adding doktersaraf:`, err);
    alert(`Gagal menambahkan data, periksa koneksi backend doktersaraf.`);
  });
});
...
editModal.id = "editdoktersarafModal";
...
editModal.innerHTML = `
  ...
  ">
    ...
    <form id="editdoktersarafForm" style="display: flex; flex-direction: column; gap: 15px;">
      ...
      <textarea id="editTitle" name="title" placeholder="Judul doktersaraf" required style="
        padding: 10px 12px;
        border: 1px solid #ccc;
        border-radius: 8px;
        font-size: 15px;
...
document.getElementById('editdoktersarafForm').addEventListener('submit', e => {
  e.preventDefault();
  ...
  fetch(`${LARAVEL_URL}/api/doktersaraf/${id}`, {
    method: 'POST',
  ...
  .then(res => res.json())
  .then(updated => {
    const idx = doktersarafData.findIndex(p => p.id == id);
    doktersarafData[idx] = updated;
    renderCards();
    editModal.style.display = "none";
  })
