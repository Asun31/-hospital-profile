...
document.addEventListener('DOMContentLoaded', () => {
  console.log("dokterobgyn.js loaded!");

  const LARAVEL_URL = "http://localhost:8000"; 
  const searchInput = document.getElementById('searchInput');
  const container = document.getElementById('dokterobgyn-dokterobgyn_m');

  let dokterobgynData = [];

  function initSection() {
    fetch(`${LARAVEL_URL}/api/dokterobgyn`)
      .then(res => res.json())
      .then(data => {
        dokterobgynData = data;
        renderCards();
      })
      .catch(err => console.error(`Error fetching dokterobgyn:`, err));
  }

  function renderCards() {
    container.innerHTML = '';

    const itemsToRender = [...dokterobgynData].sort(
    ...
    container.querySelectorAll('.delete-card').forEach(btn => {
      ...
      btn.addEventListener('click', (e) => {
        ...
        const item = dokterobgynData.find(b => b.id == id);
        if (!item) return;

        if (confirm('Apakah yakin ingin menghapus dokterobgyn ini?')) {
          fetch(`${LARAVEL_URL}/api/dokterobgyn/${item.id}`, { method: 'DELETE' })
            .then(res => {
              if (res.ok) {
                dokterobgynData = dokterobgynData.filter(b => b.id !== item.id);
                renderCards();
                alert('dokterobgyn berhasil dihapus!');
              } else {
                alert('Gagal menghapus dokterobgyn.');
              }
            })
    ...
    container.querySelectorAll('.edit-card').forEach(btn => {
      ...
      btn.addEventListener('click', (e) => {
        ...
        document.getElementById('editdokterobgynModal').style.display = 'block';
        document.getElementById('editdokterobgynForm').setAttribute('data-id', item.id);
      });
    });
  }
...
// === Add/Edit Modal ===
const addBtn = document.getElementById('adddokterobgynBtn');
const modal = document.getElementById('dokterobgynModal');
const closeBtn = document.getElementById('closedokterobgynModal');
const form = document.getElementById('dokterobgynForm');
...
form?.addEventListener('submit', e => {
  e.preventDefault();
  const img = document.getElementById('dokterobgynImg').files[0];
  const img2 = document.getElementById('dokterobgynImg2').files[0]; // Gambar tambahan
  ...
  .then(res => res.json())
  .then(newCard => {
    dokterobgynData.push(newCard);
    renderCards();
    modal.style.display = 'none';
    form.reset();
  })
  .catch(err => {
    console.error(`Error adding dokterobgyn:`, err);
    alert(`Gagal menambahkan data, periksa koneksi backend dokterobgyn.`);
  });
});
...
editModal.id = "editdokterobgynModal";
...
editModal.innerHTML = `
  ...
  ">
    ...
    <form id="editdokterobgynForm" style="display: flex; flex-direction: column; gap: 15px;">
      ...
      <textarea id="editTitle" name="title" placeholder="Judul dokterobgyn" required style="
        padding: 10px 12px;
        border: 1px solid #ccc;
        border-radius: 8px;
        font-size: 15px;
...
document.getElementById('editdokterobgynForm').addEventListener('submit', e => {
  e.preventDefault();
  ...
  fetch(`${LARAVEL_URL}/api/dokterobgyn/${id}`, {
    method: 'POST',
  ...
  .then(res => res.json())
  .then(updated => {
    const idx = dokterobgynData.findIndex(p => p.id == id);
    dokterobgynData[idx] = updated;
    renderCards();
    editModal.style.display = "none";
  })
